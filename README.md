...
static void Main(string[] args)
{
	...
	string[] vargs = { "-bool", "-int", "10", "-double", System.#2b91af">Math.PI.ToString(), "-string", "hi", "-stringArray", "Hello", "World" };
	Args.#2b91af">Args parser = new Args.#2b91af">Args(schema, vargs);
 
	#2b91af">Console.WriteLine("bool: {0}", parser.GetBoolean("bool"));
	#2b91af">Console.WriteLine("int: {0}", parser.GetInt("int"));
	#2b91af">Console.WriteLine("double: {0}", parser.GetDouble("double"));
	#2b91af">Console.WriteLine("string: {0}", parser.GetString("string"));
 
	string[] stringArray = parser.GetStringArray("stringArray");
	for (int i = 0; i < stringArray.Length; i++)
	{
		#2b91af">Console.WriteLine("stringArray {0}: {1}", i, stringArray[i]);
	}
 
	#2b91af">Console.ReadLine();
}
```
